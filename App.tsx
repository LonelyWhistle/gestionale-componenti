...
// --- TIPI DI DATI ---
interface Supplier { id: string; name: string; partNumber: string; cost: number; leadTime: string; packaging: string; }
interface LogEntry { id: string; timestamp: string; userId: string; username: string; action: string; details: string; note: string; }
interface ElectronicComponent { id: string; sekoCode: string; aselCode: string; lfWmsCode: string; description: string; suppliers: Supplier[]; logs?: LogEntry[]; }
interface BomItem { sekoCode: string; quantity: number; }
interface Product { id: string; code: string; name: string; bom: BomItem[]; logs?: LogEntry[]; }
...
const checkFirebaseConfig = (config) => Object.values(config).every(value => value && typeof value === 'string' && value.length > 0);

// --- COMPONENTI ICONE ---
const Icon = ({ children, className }) => <svg xmlns="http://www.w3.org/2000/svg" className={className || "w-5 h-5"} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>{children}</svg>;
...
const ComponentModal = ({ component, onClose, onSave }) => {
    ...
    useEffect(() => {
      ...
      } else { setFormData({ sekoCode: '', aselCode: '', description: '', suppliers: [], logs: [] }); setLfWmsCode(''); }
    }, [component]);
...
const ProductModal = ({ isOpen, onClose, onSave, product }) => {
    ...
    useEffect(() => {
        if (product) { setName(product.name); setCode(product.code); setBom(product.bom || []); } 
        else { setName(''); setCode(''); setBom([]); }
    }, [product]);
    ...
    const handleSubmit = () => { 
        ...
        onSave({ ...product, name, code, bom }); onClose(); 
    };
